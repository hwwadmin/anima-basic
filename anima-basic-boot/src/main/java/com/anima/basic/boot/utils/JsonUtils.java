...
public abstract class JsonUtils {
    ...
    public static <T> T format(String json, Class<T> tClass) {
        try {
            return objectMapper.readValue(json, tClass);
        } catch (JsonProcessingException e) {
            throw UtilsException.exception("Json字符串解析错误，解析" + tClass.getName(), e);
        }
    }
    ...
    public static <T> T format(String json, TypeReference<T> type) {
        try {
            ...
            throw UtilsException.exception("Json字符串解析错误，解析" + type.getType().getTypeName(), e);
        }
    }
    ...
    public static <T> String list2JsonStr(List<T> objects) {
        if (CollectionUtils.isEmpty(objects)) {
            return AppConstants.MARK_EMPTY_JSONARRAY;
        }
        try {
